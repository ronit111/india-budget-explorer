...
const INDICATOR_COLORS: Record<string, string> = {
  ger_primary: 'var(--blue)',
  ger_secondary: 'var(--blue-light)',
  ger_higher_sec: 'var(--blue)',
  dropout_primary: 'var(--saffron)',
  dropout_secondary: 'var(--saffron)',
  can_read_std2: 'var(--blue)',
  can_do_subtraction: 'var(--blue-light)',
  can_read_english: 'var(--blue)',
  ptr: 'var(--blue)',
  schools_computers: 'var(--blue-light)',
  schools_internet: 'var(--blue)',
  girls_toilets: 'var(--blue-light)',
};
...
function formatIndicatorValue(value: number, unit: string): string {
  ...
  if (unit === 'per teacher') return `${value.toFixed(0)}`;
  if (unit === 'students/teacher') return `${value.toFixed(0)}`;
  return Math.round(value).toLocaleString('en-IN');
}

export default function EducationExplorePage() {
  const year = useEducationStore((s) => s.selectedYear);
  const selectedCategory = useEducationStore((s) => s.selectedCategory);
  const setCategory = useEducationStore((s) => s.setSelectedCategory);
  ...
  const barItems: BarItem[] = useMemo(() => {
    ...
    return activeIndicator.states
      ...
      .map((s) => ({
        ...
        label: s.name,
        value: s.value,
        color: INDICATOR_COLORS[activeIndicator.id] || 'var(--blue)',
      }));
  }, [activeIndicator]);
                    ...
                    >
                      ...
                      {topState && (
                        <span
                          ...
                          style={{ color: INDICATOR_COLORS[ind.id] || 'var(--blue)' }}
                        >
                          Top: {topState.name}
                        </span>
                      )}
