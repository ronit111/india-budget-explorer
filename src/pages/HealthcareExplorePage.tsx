...
function formatIndicatorValue(value: number, unit: string): string {
  ...
  if (unit === 'per 1000') return `${value.toFixed(2)}`;
  if (unit === 'per lakh') return `${value.toFixed(0)}`;
  if (unit === 'per 10K') return `${value.toFixed(1)}`;
  if (unit === 'per 100K') return `${value.toFixed(0)}`;
  if (unit === 'USD') return `$${Math.round(value).toLocaleString('en-IN')}`;
  return Math.round(value).toLocaleString('en-IN');
}

export default function HealthcareExplorePage() {
  const year = useHealthcareStore((s) => s.selectedYear);
  const selectedCategory = useHealthcareStore((s) => s.selectedCategory);
  const setCategory = useHealthcareStore((s) => s.setSelectedCategory);
  ...
  const barItems: BarItem[] = useMemo(() => {
    ...
    return activeIndicator.states
      ...
      .map((s) => ({
        ...
        label: s.name,
        value: s.value,
        color: INDICATOR_COLORS[activeIndicator.id] || 'var(--rose)',
      }));
  }, [activeIndicator]);
                    ...
                    >
                      ...
                      {topState && (
                        <span
                          ...
                          style={{ color: INDICATOR_COLORS[ind.id] || 'var(--rose)' }}
                        >
                          Top: {topState.name}
                        </span>
                      )}
