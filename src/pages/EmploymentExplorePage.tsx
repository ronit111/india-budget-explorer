...
const INDICATOR_COLORS: Record<string, string> = {
  unemployment_total: 'var(--amber)',
  unemployment_youth: 'var(--saffron)',
  unemployment_female: 'var(--amber-light)',
  unemployment_male: 'var(--amber)',
  lfpr_total: 'var(--amber)',
  lfpr_male: 'var(--amber)',
...
export default function EmploymentExplorePage() {
  const year = useEmploymentStore((s) => s.selectedYear);
  const selectedCategory = useEmploymentStore((s) => s.selectedCategory);
  const setCategory = useEmploymentStore((s) => s.setSelectedCategory);
  ...
  const barItems: BarItem[] = useMemo(() => {
    ...
    return activeIndicator.states
      ...
      .map((s) => ({
        ...
        label: s.name,
        value: s.value,
        color: INDICATOR_COLORS[activeIndicator.id] || 'var(--amber)',
      }));
  }, [activeIndicator]);
    ...
    >
      ...
      >
        ...
        <motion.p
          ...
          variants={fadeUp}
        >
          Select a category and indicator to compare states. Data from PLFS 2023-24, World Bank, and RBI KLEMS.
        </motion.p>
                    ...
                    >
                      ...
                      {topState && (
                        <span
                          ...
                          style={{ color: INDICATOR_COLORS[ind.id] || 'var(--amber)' }}
                        >
                          Top: {topState.name}
                        </span>
                      )}
