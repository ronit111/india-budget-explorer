...
export function EmploymentHeroSection({ summary }: EmploymentHeroSectionProps) {
  const [ref, isVisible] = useScrollTrigger({ threshold: 0.2 });

  const formattedWorkforce = summary
    ? `${Math.round(summary.workforceTotal)} crore`
    : '—';
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
        ...
        >
          <div className="text-hero gradient-text-amber">
            {formattedWorkforce}
          </div>
          <p
          ...
          >
            workers power the Indian economy
          </p>
        </motion.div>
